...
namespace Proyecto.App.Persistencia
{
    public class RepositorioOperador : IRepositorioOperador
    {
        ...
        {
            var Encontrado = _appContext.operador.FirstOrDefault(p => p.IdCodOperador == del); //IdCodOperador es la pk de Operador.cs
            if (Encontrado == null)
                return;
        ...
        {
            return _appContext.operador.FirstOrDefault(p => p.IdCodOperador == get);
        }
        Operador IRepositorioOperador.UpdateOperador(Operador up)
        {
            var Encontrado = _appContext.operador.FirstOrDefault(p => p.IdCodOperador == up.IdCodOperador);
            if (Encontrado != null)
            {
